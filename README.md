...
This repository contains a constructive certificate for a 1-factorization of
the 6-dimensional hypercube `Q_6` whose Hamilton-pair auxiliary graph is exactly
`K_{3,3}`.
...
- `q6_k33_paper.tex`: expanded LaTeX short-paper draft.
...
- `q6_k33_selfcheck.py`: self-contained verifier with the compact certificate
  table hard-coded.
- `q6_k33_artifact_readme.md`: artifact and verification notes.
...
Both checks should report that the Hamilton-pair graph is exactly the complete
bipartite graph with parts `{0,1,5}` and `{2,3,4}`.

## Manuscript

The current manuscript draft includes the explicit construction, a verification
method section, a reproducibility statement, and bibliographic context for
semi-perfect 1-factorizations.

## Citation Background

Natalie C. Behague, *Semi-perfect 1-factorizations of the hypercube*, Discrete
Mathematics 342(6) (2019), 1696-1702. DOI:
10.1016/j.disc.2019.01.035; arXiv:1811.06389.
